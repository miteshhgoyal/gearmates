            ...
            try {
                await Promise.all(
                    existingProduct.image.slice(0, newImages.length).map(async (imageUrl) => {
                        ...
                        await cloudinary.uploader.destroy(`gearmates-products/${publicId}`)
                    })
                )
...
const searchProducts = async (req, res) => {
    try {
        const { query, category, minPrice, maxPrice, brand } = req.query
        ...
        if (query) {
            searchFilter.$or = [
                ...
                { specifications: { $in: [new RegExp(query, 'i')] } }
            ]
        }
        ...
        const products = await productModel.find(searchFilter).sort({ date: -1 })
